package com.districthut.islam.Activities.Quran.util;
...
public class MediaDownloadManager {
    ...
    public static long startDownload(Context context, String title, String url, String path) {
        Uri source = Uri.parse(url);
        Uri destination = Uri.fromFile(new File(path));
        Request request = new Request(source);
        request.setAllowedNetworkTypes(3);
        ...
        request.setVisibleInDownloadsUi(true);
        request.setTitle(title);
        request.setDestinationInExternalFilesDir(context, Environment.getRootDirectory().getAbsolutePath(),path);
        request.setDestinationUri(destination);
        request.setNotificationVisibility(1);
        return downloadManager.enqueue(request);
    }
